<template>
    <section class="flex flex-col md:flex-row justify-between" ref="statsSection">
        <div v-for="(item, index) in stats" :key="index" :class="[
            'p-6 h-36 md:h-72 w-full md:w-1/4 flex items-center md:justify-center',
...
const stats = ref([
    { to: 60, label: 'Years Experience', count: 0 },
    { to: 1090, label: 'Total Cars', count: 0 },
    { to: 2590, label: 'Happy Clients', count: 0 },
...
const startCounting = () => {
    stats.value.forEach((stat) => {
        let start = 0
        const increment = stat.to / (2000 / 16) // ~16ms interval for smooth animation
